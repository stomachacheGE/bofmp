...
def tracking_f1_score(caculate_at, P_occ_obser, P_occ_pred):
    locs = np.where(caculate_at)
    P_occ_obser = P_occ_obser[locs]
    P_occ_pred = P_occ_pred[locs]

    # if occupancy is bigger than 0, make it a positive example
    h_max = P_occ_pred.max()/2
    P_occ_pred = np.where(P_occ_pred>1e-2, 1, 0)
    return f1_score(P_occ_obser, P_occ_pred)

def tracking_ap_score(caculate_at, P_occ_obser, P_occ_pred):
    locs = np.where(caculate_at)
    P_occ_obser = P_occ_obser[locs]
    P_occ_pred = P_occ_pred[locs]

    return average_precision_score(P_occ_obser, P_occ_pred)
